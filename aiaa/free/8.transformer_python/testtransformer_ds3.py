...
class MLA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        ...
        kv = kv.view(bsz, seqlen, self.n_local_heads, self.head_dim)
        # 将键张量拆分为低秩键和旋转位置键
        kv, k_pe = torch.split(kv, [self.qk_nope_head_dim, self.qk_rope_head_dim], dim=-1)
        # 应用旋转位置嵌入
        ...
        # 计算注意力分数
        scores = (torch.einsum("bshd,bthd->bsht", q_nope, kv) +
                  torch.einsum("bshr,btr->bsht", q_pe, k_pe)) * self.head_dim ** -0.5
        # 如果掩码不为None
        if mask is not None:
...
class MoE(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, (route, score) in enumerate(zip(routes.t(), scores.t())):
            ...
            # 计算输出
            y += score * expert(x_reshaped[route])
        # 重塑输出
        y = y.view(B, L, D)
        # 返回结果
        return y
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int = 0):
        ...
        Args:
            tokens (torch.Tensor): 输入词元。
            start_pos (int): 开始位置。默认为0。
