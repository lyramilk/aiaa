今天打算学习transformer，找到一个不错的实现，其中的ipynb是jupyter notebook格式的笔记文件，这个笔记中包含了文档和实现transformer的代码。
```
https://github.com/harvardnlp/annotated-transformer
```
然后很快就被现实打败了，英文水平有限完全不知所云。那么尝试一下翻译它吧。网上尝试找翻译插件，结果运行网上给的命令有错误。硬着头皮用文本工具打开了其中的ipynb，发现这不就是个json嘛，格式这么清楚，用不着插件了，让deepseek帮我来翻译一下吧。
...
运行翻译
```
python3 translate.py  AnnotatedTransformer.ipynb
```
然后得到了中文版的``AnnotatedTransformer.zh_CN.ipynb``，在jupter中打开看看，完成正常。
感谢 nlp.seas.harvard.edu
感谢 deepseek
让我这个学渣也有机会学习一下transformer了。